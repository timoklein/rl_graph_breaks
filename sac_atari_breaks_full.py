...
def parse_args():
    ...
    parser.add_argument("--wandb-project-name", type=str, default="compile_test",
    ...
    for global_step in range(args.total_timesteps):
        ...
        if global_step > args.learning_starts:
            if global_step % args.update_frequency == 0:
                ...
                results_critic = dynamo.explain(
                    critic_update, actor, qf1, qf2, qf1_target, qf2_target, q_optimizer, alpha, data, args.gamma
                )

                # CRITIC training
                qf1_a_values, qf2_a_values, qf1_loss, qf2_loss, qf_loss = critic_update(
                    actor, qf1, qf2, qf1_target, qf2_target, q_optimizer, alpha, data, args.gamma
                )

                # ACTOR training
                log_pi, action_probs, actor_loss = actor_update(actor, qf1, qf2, actor_optimizer, alpha, data)

                results_actor = dynamo.explain(actor_update, actor, qf1, qf2, actor_optimizer, alpha, data)

                results_alpha = dynamo.explain(alpha_update, target_entropy, log_alpha, a_optimizer, log_pi, action_probs)

                if args.autotune:
                    # re-use action probabilities for temperature loss
                    alpha, alpha_loss = alpha_update(target_entropy, log_alpha, a_optimizer, log_pi, action_probs)
                    alpha = alpha.item()
